...
TODO : explication estimation densité ; explication validation / apprentissage / test ; mlr3 ; NB exo 2 best model ; exo 3 ; scale => utiliser valeur de train pour le test
...
On définie la règle de Bayes en calculant $P(Yi=1|X_i=x_i)$ selon les valeurs de $X$.

Pour $X_i \leq 0$ : $P(Y_i=1|X_i=x_i)=P(U_i \leq 0,1) = 0,1$ d'où $f^*(x_i) = 0$.

Pour $X_i > 0$ : $P(Yi=1|X_i=x_i)=P(U_i > 0,2) = 1 - P(U_i \leq 0,2) = 0,8$ d'où $f^*(x_i) = 1$.
...
Pour $X_i \leq 0$ : $P(Yi=1|X_i=x_i)=P(U_i \leq 0,2) = 0,2$ d'où $f^*(x_i) = 0$.

Pour $X_i > 0$ : $P(Yi=1|X_i=x_i)=P(U_i > 0,4) = 1 - P(U_i \leq 0,4) = 0,6$ d'où $f^*(x_i) = 1$.
...
Le meilleur $k$ est obtenu avec la commande `knn_cross_results$best.parameters$k` : `r knn_cross_results$best.parameters$k`. De cette manière le choix de $k$ n'est plus arbitraire mais basé sur nos données et le $k$ donnant l'erreur minimale (sur les données de validation) est choisie. 
...
Comme dans le cas binaire c'est le maximum a posteriori qui est utilisé pour choisir la classe prédite.
...
On peut visualiser les résultats sous forme graphique.
...
knn_fit <- train(
  ...
  tuneGrid = data.frame(k = seq(100, 200, by = 10)), # nombre de voisins
...
L'erreur est obtenue avec la commande suivante `sqrt(mean((knn_pred - test$price)^2))`. Elle est de `r round(sqrt(mean((knn_pred - test$price)^2)), digits = 3)`.

#### Variables catégorielles encodées

Les 3 variables catégorielles vont être encodées en variables continues. Les modalités sont transformées en nombre. Il faut essayer de faire en sorte de ranger les modalités par ordre d'importance si possible. 
