...
Pour $x_i \leq 0$ : $P(Y=1|X=x_i)=P(U_i \leq 0,1) = 0,1$ d'où $f^*(x_i) = 0$.

Pour $x_i > 0$ : $P(Y=1|X=x_i)=P(U_i > 0,2) = 1 - P(U_i \leq 0,2) = 0,8$ d'où $f^*(x_i) = 1$.
...
$$
\underset{y \in Y}{\arg\max} \ \ \widehat{P}(Y=y|X=x)
$$

Comme $P(X=x)$ ne dépend pas de $y$ on cherche donc :

$$
\underset{y \in Y}{\arg\max} \ \ \widehat{P}(Y=y) \times \widehat{P}(X=x|Y=y)
$$
...
$$
\underset{y \in Y}{\arg\max} \ \ \widehat{P}(Y=y) \times \prod_{j=1}^{J}\widehat{P}(X_j=x_j|Y=y)
$$
Les probabilités renvoyées dans les différentes implémentations de ce modèle correspondent aux $\widehat{P}(Y=y) \times \prod_{j=1}^{J}\widehat{P}(X_j=x_j|Y=y)$ pour chaque $y \in Y$ et non pas à $\widehat{P}(Y=y|X=x)$. 
...
On effectue une ACM sur les données d'apprentissage en conservant toutes les dimensions dans le résultat. On transforme ainsi les variables _neighbourhood_group_ et _room_type_ en variables continues. (La variable _neighbourhood_ n'est pas utilisée car il faudrait effectuement un traitement sur les modalités à faible fréquence pour éviter des problèmes lors de la projection des indidivus de test sur les axes).
...
On a testé 3 approches pour répondre à la problémaique de prédiction. On peut voir les différentes erreurs obtenues dans le tableau suivant. 
